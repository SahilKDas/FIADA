...
N=len(COURSE); OBS=14; H=12; OUT=4; PARAMS=OBS*H+H+H*OUT+OUT
...
def evaluate(w,steps=1100):
 ...
 for _ in range(steps):
  ...
  obs=np.stack([np.sin(err),np.cos(err),np.clip(lateral/68,-2,2),speed/45,vy/15,yaw/2,curve,road,np.zeros(pop),np.zeros(pop),np.zeros(pop),np.zeros(pop),np.zeros(pop),np.zeros(pop)],1)
...
def main():
 ...
 obs[:,2]*=1.4; obs[:,3]=rng.uniform(0,1.25,samples); obs[:,4:6]*=.8; obs[:,6]*=.8; obs[:,7]=1; obs[:,8:14]=0
 ...
 target=np.stack([throttle,np.arctanh(steer),np.full(samples,-4.5),np.full(samples,-3.0)],1)
 lr=.018
